...
+ res (reserved), checing resosultion, how frequent to check on the sensor data 
...
To train the classifier on your own photos, you need to place enough photos of yours in .png format (>3 is recommended) in /srv/openface/data/mydataset/raw/YOUR_NAME/ and run ./train.sh -- In a few minutes the classifer will be able to recognize your face. Note that this is done inside the docker container (i.e., run ./start.sh first)
