...
def makePredictions(sensor):
    ...
    clf.fit(selectedFeatures, data.labels)

    meta = readPredMeta(sensor)
    features = generatePredictionData(meta, predictionDir(sensor))
    scaledPredictionFeatures = scaler.transform(features)
    selectedPredictionFeatures = selector.transform(scaledPredictionFeatures)

    print selectedPredictionFeatures
    
    predictions = clf.predict(selectedPredictionFeatures)
    print predictions
...
if __name__ == "__main__":
    ...
    #makePredictions(sensor)

    #scores = crossValidation(sensor)
    #print scores
