...
class DataStoreThread(threading.Thread):
    ...
    def run(self):
        ...
        for i in range(self.n):
            time.sleep(self.t)
            self.data[self.count,0] = current_milli_time()
            self.data[self.count,1:14] = latestReadings[0,0:13]
            self.count = self.count + 1
            if not os.path.exists(SAMPLING_FLAG_PATH):
            	break;
           
        print "Sampling finished"

        self.saveSample(self.command)
        #setSamplingRates(samplingRates)
        if os.path.exists(SAMPLING_FLAG_PATH):
        	os.remove(SAMPLING_FLAG_PATH)
    ...
    def saveSample(self, cmd):
		# Check if sensor directry exists. If not create
		dataDir = 'sensors/'+cmd['sensor']
		if cmd['command'] == 'sample':
			dataDir = dataDir +'/samples/'
		else:
			dataDir = dataDir +'/predictions/'

		if not os.path.exists(dataDir):
			os.makedirs(dataDir)

		if not os.path.exists(dataDir + 'meta.json'):
			meta = json.dumps([])
		...
		np.savetxt(dataDir + filename, self.data[0:self.count,:], delimiter=',')
		...
		print "Sampled " + str(self.count) + " data points to " + dataDir + filename
...
if __name__ == "__main__":
	...
	while True:
		...
		# Process commands
		if not os.path.exists('cmd/lock'):
			for path in glob.glob('cmd/*'):
				# Read JSON command
				cmd = json.loads(open(path).read())

				print "Executing \"" + cmd['command'] + "\" command"
				...
				dataStoreThread.setCommand(cmd)
				open(SAMPLING_FLAG_PATH, 'a').close()
